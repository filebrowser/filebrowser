...
var sharePostHandler = withPermShare(func(w http.ResponseWriter, r *http.Request, d *data) (int, error) {
	...
	shareHash, autogenerated, err := getShareHash(body.Hash)
	if err != nil {
	...
	for attempts := 0; attempts < 32; attempts++ {
		if err := d.store.Share.Save(s); err != nil {
			if errors.Is(err, fberrors.ErrExist) {
				if !autogenerated {
					http.Error(w, "share key already exists", http.StatusConflict)
					return 0, nil
				}

				shareHash, autogenerated, err = getShareHash("")
				if err != nil {
					return http.StatusInternalServerError, err
				}
				s.Hash = shareHash
	...
	if !saved {
		return http.StatusInternalServerError, fmt.Errorf("failed to allocate share key")
	}
...
func getShareHash(requested string) (string, bool, error) {
	requested = strings.TrimSpace(requested)
	if requested != "" {
		hash, err := share.ValidateHash(requested)
		return hash, false, err
	}

	hash, err := share.GenerateHash()
