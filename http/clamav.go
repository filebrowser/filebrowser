...
func normalizeClamAVServiceMessage(message string) string {
	...
	if strings.Contains(normalized, "file size limit exceeded") ||
		strings.Contains(normalized, "max size of uploaded file") ||
		strings.Contains(normalized, "request entity too large") ||
		strings.Contains(normalized, "payload too large") {
		return "The configured ClamAV scanner API rejected the file because its upload/stream size limit was reached. File Browser does not enforce this limit and now streams scans without a fixed client timeout. Increase or remove the size limit on the ClamAV API/reverse proxy/clamd server, then scan again. Scanner response: " + message
	}
...
func scanWithClamAV(ctx context.Context, cfg settings.ClamAV, filename string, reader io.Reader) error {
	...
	// Do not set a client-level timeout here. Large manual scans and large
	// upload scans can legitimately take longer than a fixed deadline. The
	// request context still controls cancellation when the caller disconnects
	// or when a higher-level handler cancels the operation.
	client := &http.Client{}
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		...
		return &clamAVServiceError{Message: normalizeClamAVServiceMessage(message)}
	}
