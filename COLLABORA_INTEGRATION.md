...
| `--collabora.publicURL` | `FB_COLLABORA_PUBLIC_URL` | External/public File Browser base URL that Collabora can call back to, for example `https://filebrowser.koszhome.ro`. Include `baseURL` here if File Browser is mounted under a path. |
| `--collabora.internalURL` | `FB_COLLABORA_INTERNAL_URL` | Optional internal/LAN File Browser base URL used when File Browser is opened from LAN, for example `http://192.168.68.204:8080`. |
...
services:
  filebrowser:
    ...
    environment:
      ...
      FB_COLLABORA_URL: https://collabora.koszhome.ro
      FB_COLLABORA_PUBLIC_URL: https://filebrowser.koszhome.ro
      FB_COLLABORA_INTERNAL_URL: http://192.168.68.204:8080
...
```text
http://192.168.68.204:8080
https://filebrowser.koszhome.ro
```
...
```yaml
environment:
  aliasgroup1: 'https://filebrowser\.koszhome\.ro:443'
  aliasgroup2: 'https://filebrowser\.koszhome\.ro'
  aliasgroup3: 'http://192\.168\.68\.204:8080'
```
